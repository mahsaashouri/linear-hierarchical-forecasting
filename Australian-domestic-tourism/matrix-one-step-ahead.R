## Here we compute forecasts using matrix computation and we will compare it with the results by separate regression models - Rolling forecasts
library(tidyverse)
library(hts)
source("olsfc.R")
library(Matrix)
...
## computing predictor matrix (trend, dummy seasonality, lags) for each series
Xmat<-list()
freq <-12
maxlag <- 12
